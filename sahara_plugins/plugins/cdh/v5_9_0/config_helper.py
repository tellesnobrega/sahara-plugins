...
from sahara_plugins.plugins.cdh import config_helper as c_h


class ConfigHelperV590(c_h.ConfigHelper):
    path_to_config = 'plugins/cdh/v5_9_0/resources/'

    CDH5_UBUNTU_REPO = (
        'deb [arch=amd64] http://archive.cloudera.com/cdh5'
        '/ubuntu/trusty/amd64/cdh trusty-cdh5.9.0 contrib'
        '\ndeb-src http://archive.cloudera.com/cdh5/ubuntu'
        '/trusty/amd64/cdh trusty-cdh5.9.0 contrib')
    ...
    CM5_UBUNTU_REPO = (
        ...
        '/ubuntu/trusty/amd64/cm trusty-cm5.9.0 contrib'
        '\ndeb-src http://archive.cloudera.com/cm5/ubuntu'
        '/trusty/amd64/cm trusty-cm5.9.0 contrib')
    ...
    HIVE_SERVER2_SENTRY_SAFETY_VALVE = utils.get_file_text(
        path_to_config + 'hive-server2-sentry-safety.xml', 'sahara_plugins')

    HIVE_METASTORE_SENTRY_SAFETY_VALVE = utils.get_file_text(
        path_to_config + 'hive-metastore-sentry-safety.xml', 'sahara_plugins')

    SENTRY_IMPALA_CLIENT_SAFETY_VALVE = utils.get_file_text(
        path_to_config + 'sentry-impala-client-safety.xml', 'sahara_plugins')
